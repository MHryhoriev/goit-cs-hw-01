...
class Parser:
    ...
    def term(self):
        """Парсер для множення та ділення (term)."""
        node = self.factor()
    ...
    def factor(self):
        """ Парсер для чисел і виразів у дужках. """
        token = self.current_token
...
class Interpreter:
    ...
    def interpret(self):
        ...
        try:
            ...
            if tree is None:
                raise Exception('Помилка: порожнє дерево виразу')
            return self.visit(tree)
        except Exception as e:
            print(f'Помилка при інтерпретації: {str(e)}')

    def visit(self, node):
        """Відвідує вузол та викликає відповідний метод для кожного типу вузла."""
        if node is None:
            raise Exception('Помилка: наданий вузол є None')
